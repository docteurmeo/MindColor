...
- **Symbol/icon hơn wordmark.** Khi logo có cả symbol và text, ưu tiên symbol-only (mảnh dễ nhớ về màu). Nếu chưa có SVG symbol-only sạch thì mark `Pending`, không giữ lockup/wordmark để đố tạm.
- **SVG là CSS mask** — chỉ alpha channel quan trọng. Fill trong SVG phải là màu brand chính thức (build script extract từ `fill=...`).
- **Audit trước khi merge brand.** Chạy `node scripts/audit-brands.mjs`; kết quả hợp lệ là `flaggedCount: 0`.
- **Current pool:** 103 brand sau audit symbol-only / multi-color cleanup ngày 2026-05-19.
...
│   ├── audit-brands.mjs         # Kiểm tra SVG rỗng/missing/multi-color/raster/style/root fill lệch
...
- [x] Session 8.5 — Brand audit cleanup: symbol-only, remove multi-color/raster/wrong assets, pool: 103
